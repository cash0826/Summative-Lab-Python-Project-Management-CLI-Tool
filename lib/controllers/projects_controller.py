...
class ProjectsControllers():
  ...
  def list_projects_for_user(self, args):
    user_projects = [project for project in self.data if project.owner_email == args.user_email]
    # check email exists in users.json
    users = storage.load_data("./data/users.json")
    if not any(user["email"] == args.user_email for user in users):
      print(f"Error: No user found with email {args.user_email}.")
      return []
    
    # check if user has any projects
    if not user_projects:
      print(f"No projects found for user with email {args.user_email}.")
      return []
